...
/*   Updated: 2021/02/03 16:08:59 by ezachari         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_read_file(int fd, char **fdb)
{
	...
	if (*fdb == NULL)
		*fdb = ft_strdup_gnl("");
	while (!(ft_strchr_gnl(*fdb, '\n')))
	{
		if ((rbyte = read(fd, buffer, BUFFER_SIZE)) < 0)
			return (NULL);
		buffer[rbyte] = '\0';
		leakfdb = ft_strjoin_gnl(*fdb, buffer);
...
static int	ft_wr(char *eos, char **line, char **fdbuf, int fd)
{
	...
	leakline = *line;
	*line = ft_substr_gnl(fdbuf[fd], 0, eos - fdbuf[fd]);
	free(leakline);
	leakfd = fdbuf[fd];
	fdbuf[fd] = ft_strdup_gnl(fdbuf[fd] + (eos - fdbuf[fd]) + 1);
	free(leakfd);
	return (0);
}
